...
function SimnodesDashboard() {
  ...
  const osIdx = useMemo(() => columns.indexOf('operatingSystem'), [columns]);

  useMessageListener<SimnodesMessage>(useCallback((msg) => {
  ...
  const handleViewYaml = useCallback((row: unknown[]) => {
  ...
  }, [postMessage, nameIdx, nsIdx, osIdx]);
            ...
            {filteredAndSortedRows.map((row, rowIdx) => (
              <tr key={rowIdx} className="hover:bg-[var(--vscode-list-hoverBackground)]">
                ...
                {columns.map((col, colIdx) => {
                  ...
                  const statusClass = col === 'pod-status' ? getStatusClassName(value) : '';
                  return (
                    <td key={colIdx} className={`border border-[var(--vscode-panel-border)] px-2 py-1 whitespace-pre ${statusClass}`}>
                      {value}
                    </td>
                  );
                })}
